...
def read_genomic_ranges(genomic_ranges_file: str) -> dd.DataFrame:
    ...
    """
    print("genomic ranges file", genomic_ranges_file)
    ...
    genomic_ranges['chr'] = genomic_ranges['chr'].astype(str)
    return genomic_ranges

def read_ribo_seq_bed(signal_file: str) -> dd.DataFrame:
    """
...
def read_ribo_seq_bigwig(signal_file: str) -> pyBigWig:
    """
...
def calculate_signal_support_bigwig(
    ...
    print("GENOMIC RANGES: ", genome_ranges)
    ...
    print("SIGNAL", signal_support_list)
...
def write_output_pd(signal_support: dd.DataFrame, output_file: str):
    ...
    signal_support.to_csv(output_file, index=False)
...
def main(genomic_ranges_file: str, signal_file: str, output_file: str, cutoff: int = 50, format: str = None):
    """
    Run checker 
    """
    ...
    if not format:
        file_is_bw = pyBigWig.open(signal_file).isBigWig()
    else:
        file_is_bw = format == "bw"

    if file_is_bw:
        ribo_seq = read_ribo_seq_bigwig(signal_file)
        ...
        ribo_seq = read_ribo_seq_bed(signal_file)
...
if __name__ == "__main__":
    ...
    if args.low_memory:
        ...
        main(args.genomic_ranges_file, args.signal_file, args.output_file, args.cutoff, args.format)
